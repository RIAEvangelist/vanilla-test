...
export function summarizeResult(value, runtime, source = 'test entry') {
    ...
    const passed = Array.isArray(result.passed)
        ? result.passed.filter((entry) => typeof entry === 'string').map(stripVTControlCharacters)
        : [];
    const failed = Array.isArray(result.failed)
        ? result.failed.filter((entry) => typeof entry === 'string').map(stripVTControlCharacters)
        : [];
