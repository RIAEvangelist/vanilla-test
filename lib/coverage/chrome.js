...
export async function runChromeCoverage(config, signal) {
    ...
    const outputDirectory = path.join(config.reportsDirectory, 'chrome');
    ...
    const tempDirectory = await fs.mkdtemp(path.join(os.tmpdir(), 'vanilla-test-chrome-'));
    const tempScreenshot = path.join(tempDirectory, 'vanilla-test-chrome.png');
    let server;
    let browser;
    let page;
    ...
    try {
        ...
        return result.ok && !thresholdFailure ? 0 : 1;
    } catch (error) {
