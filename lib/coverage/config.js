...
export function loadConfig(configArgument, overrides = {}) {
    ...
    const nodeSource = object(source.node, 'node');
    rejectUnknown(nodeSource, NODE_KEYS, 'node');
    const chromeSource = object(source.chrome, 'chrome');
    rejectUnknown(chromeSource, CHROME_KEYS, 'chrome');
    ...
    if (overrides.chromePath !== undefined) {
        string(overrides.chromePath, '--chrome-path');
    }

    const configuredImports = browserImports(root, chromeSource.imports ?? {});
    const timeoutMs = overrides.timeoutMs === undefined
        ? timeout(source.timeoutMs ?? 30_000)
        : timeout(overrides.timeoutMs, '--timeout-ms');

    const nodeInclude = includes(nodeSource.include, 'node.include');
    const chromeInclude = includes(chromeSource.include, 'chrome.include');
    matchedFiles(root, nodeInclude, 'node.include');
    matchedFiles(root, chromeInclude, 'chrome.include');
    ...
    return Object.freeze({
        ...
        chrome: Object.freeze({
            ...
            imports: Object.freeze({ ...defaultImports(root), ...configuredImports }),
            headless: overrides.headed ? false : (chromeSource.headless ?? true),
            executablePath: overrides.chromePath
